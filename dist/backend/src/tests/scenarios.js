...
async function runBrowseAndBidChecks(context) {
    ...
    (0, assert_1.assert)(Array.isArray(properties.data) && properties.data.length > 0, "Properties should not be empty");
    (0, assert_1.assertIncludes)(properties.data, (property) => property.status === "ACTIVE", "At least one active property should exist");
    ...
    (0, assert_1.assert)(Array.isArray(searchResult.data) && searchResult.data.every((property) => property.city.includes("Dubai") || property.location.includes("Dubai") || property.title.includes("Dubai")), "Search result should only include matching Dubai properties");
    const activeProperty = properties.data.find((property) => property.status === "ACTIVE");
...
async function runAdminChecks(context) {
    ...
    const upload = await (0, test_client_1.apiRequest)("/api/admin/upload", {
        method: "POST",
        token: context.adminToken,
        formData: (0, test_client_1.createTextUploadForm)("smoke-upload.txt", "estatebid smoke upload"),
    });
